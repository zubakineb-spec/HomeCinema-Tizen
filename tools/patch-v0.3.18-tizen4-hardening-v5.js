...
if (!app.includes('  seekBusy:false,')) {
  app = replaceOnce(app, '  saveTimer:null,\n', '  saveTimer:null,\n  seekBusy:false,\n  pendingStop:null,\n', 'state fields');
}
...
if (!app.includes(clickSeekNew)) app = replaceOnce(app, clickSeekOld, clickSeekNew, 'click seek callbacks');
...
if (!app.includes(remoteRewindNew)) app = replaceOnce(app, remoteRewindOld, remoteRewindNew, 'remote rewind callback');
...
if (!app.includes(remoteForwardNew)) app = replaceOnce(app, remoteForwardOld, remoteForwardNew, 'remote forward callback');
...
[
  ...
  "seek(-10000,function(){showPlayerMenu('[data-player-action=\\\"rewind\\\"]')})",
  "seek(10000,function(){showPlayerMenu('[data-player-action=\\\"forward\\\"]')})"
].forEach(x => assertContains(combined, x, x));

const tmpApp = appPath + '.hardening.tmp.js';
const tmpShim = shimPath + '.hardening.tmp.js';
...
console.log('TIZEN4_HARDENING_V5=PASS');
