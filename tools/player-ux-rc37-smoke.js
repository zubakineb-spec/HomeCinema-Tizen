...
function main() {
  ...
  assert(source.includes('var SCRUB_INITIAL_RELEASE_MS=1100;'), 'initial no-keyup fallback must survive the first Samsung auto-repeat delay');
  assert(source.includes('var SCRUB_REPEAT_RELEASE_MIN_MS=520;'), 'adaptive held-release minimum is missing');
  assert(source.includes('var SCRUB_REPEAT_RELEASE_MAX_MS=1400;'), 'adaptive held-release maximum is missing');
  assert(source.includes('function adaptiveReleaseDelay()'), 'adaptive repeat-gap release is missing');
  ...
  assert(source.includes('armReleaseFallback(adaptiveReleaseDelay());'), 'adaptive repeat-gap fallback must refresh while held');
  assert(source.includes('if(scrubReleaseDeadline>0&&nowMs()>=scrubReleaseDeadline){releaseScrubFromSilence();return}'), '80ms scrub clock must also enforce release deadline');
  ...
  assert(source.includes("ui.fill.style.opacity='1'"), 'purple target fill must be restored on scrub');
  assert(source.includes('function resumeAfterSeek(p,resume,attempt)'), 'post-seek autoresume retry is missing');
  ...
  console.log('PASS: Samsung repeat confirms hold; held Left/Right moves the target on the internal 80ms clock');
  console.log('PASS: adaptive repeat-stream silence commits without requiring DOM keyup or OK');
  console.log('PASS: purple target surface is restored and playback is retried after seek');
  console.log('PASS: every scrub still has exactly one absolute seekTo');
  console.log('HOME_CINEMA_PLAYER_UX_RC325_SMOKE=PASS');
}
