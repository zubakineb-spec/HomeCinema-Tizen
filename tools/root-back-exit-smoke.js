...
if(!release.includes('function seriesPageOpen()'))fail('RC3.10 series-page guard is missing');
if(!release.includes('!seriesPageOpen()'))fail('open RC3.10 series page is not excluded from root exit');
...
if(!series.includes("if(code===10009||code===27){consume(e);closeSeriesPage();return false}"))fail('series page Back does not close the series page');

console.log('PASS: Back exits Home Cinema only from the true root screen');
console.log('PASS: player, details, search, about and RC3.10 series page retain nested Back behavior');
