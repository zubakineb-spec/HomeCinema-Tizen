...
func (s *Server) routes() {
	...
	s.mux.HandleFunc("/api/next", s.nextEpisode)
	s.mux.HandleFunc("/api/image", s.imageCache)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	jsonOut(w, map[string]any{
		...
		"media_base_url": s.cfg.MediaBaseURL, "media_root": s.cfg.MediaRoot,
		"tmdb": s.tmdb.enabled(), "image_cache": true, "image_cache_entries": countImageCache(s.cfg.ImageCacheDir),
		"target_nas": "QNAP D1 ARMv7 / QTS 4.3.6", "target_tv": "Samsung UE49NU7500U",
		"ffmpeg": tool("ffmpeg"), "ffprobe": tool("ffprobe"), "dts_fallback_enabled": s.cfg.EnableDTSFallback,
	})
}
...
func (s *Server) scan(w http.ResponseWriter, r *http.Request) {
	...
	jsonOut(w, map[string]any{
		"source": s.cfg.MediaRoot, "video_files": len(m) + len(ep), "movies": len(m), "shows": len(sh),
		"episodes": episodeCount, "extras": extraCount, "ignored": 0,
		"scan_reused": stats.Reused, "scan_profiled": stats.Profiled, "scan_removed": stats.Removed,
		"tmdb_enabled": s.tmdb.enabled(), "movies_matched": matched["movies_matched"],
		"shows_matched": matched["shows_matched"], "episodes_matched": matched["episodes_matched"],
	})
}
...
func (s *Server) enrich() map[string]int {
	...
	for i := range st.Shows {
		...
		if needsShowLookup {
			...
			var e error
			if preferredID > 0 {
				d, e = s.tmdb.ShowByID(preferredID)
			} else {
	...
	if q != "" {
		...
		for _, sh := range st.Shows {
			if strings.Contains(strings.ToLower(sh.Title+" "+sh.RecognizedTitle+" "+sh.OriginalTitle), q) {
				...
				x["season_count"] = seasonCount
				x["extra_count"] = extraCount
...
func (s *Server) movie(w http.ResponseWriter, r *http.Request) {
	...
	for _, m := range s.store.Snapshot().Movies {
		if m.ID == id {
			jsonOut(w, m)
			return
		}
...
func (s *Server) show(w http.ResponseWriter, r *http.Request) {
	...
	for _, n := range nums {
		seasons = append(seasons, map[string]any{"number": n, "episode_count": len(seasonMap[n]), "episodes": seasonMap[n]})
	}
	res["seasons"] = seasons
	jsonOut(w, res)
}
...
func originalSource(v string) string {
	if u, err := http.NewRequest("GET", v, nil); err == nil && strings.HasSuffix(u.URL.Path, "/api/playback/smart") {
		if x := u.URL.Query().Get("source_url"); x != "" {
			return x
		}
	}
	return v
...
func (s *Server) progress(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if err := s.store.SetProgress(p); err != nil {
			jsonErr(w, 500, err.Error())
			return
		}
...
func (s *Server) resolve(source string) (map[string]any, error) {
	if !strings.HasPrefix(source, s.cfg.MediaBaseURL) {
		return nil, fmt.Errorf("outside MEDIA_BASE_URL")
	}
	dtsOnly, probed := probe(s.cfg, source)
	if !probed {
		return map[string]any{"mode": "direct", "play_url": source, "source_url": source, "reason": "probe_unavailable"}, nil
	}
	if !dtsOnly {
		return map[string]any{"mode": "direct", "play_url": source, "source_url": source, "reason": "compatible_or_alternate_audio"}, nil
	}
	if !s.cfg.EnableDTSFallback {
		return map[string]any{"mode": "direct", "play_url": source, "source_url": source, "reason": "dts_only_fallback_disabled"}, nil
	}
	path, e := ensureHLS(s.cfg, source)
	if e != nil {
		return map[string]any{"mode": "direct", "play_url": source, "source_url": source, "reason": "dts_only_ffmpeg_unavailable"}, nil
	}
...
func (s *Server) playbackResolve(w http.ResponseWriter, r *http.Request) {
	...
	if e != nil {
		jsonErr(w, 400, e.Error())
		return
	}
	if p, ok := x["play_url"].(string); ok && strings.HasPrefix(p, "/hls/") {
		x["play_url"] = "http://" + r.Host + p
	}
	jsonOut(w, x)
}
