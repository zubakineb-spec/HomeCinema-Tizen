package app

import "crypto/x509"

// RC3.31: Let's Encrypt moved public TLS issuance to the Generation Y
// hierarchy in 2026. Older QTS/Go trust stores do not contain Root YR/YE yet.
// Keep certificate and hostname verification enabled, but explicitly trust the
// official ISRG Generation Y roots for TMDB image CDN connections.
//
// Root fingerprints (SHA-256), from ISRG / Let's Encrypt:
//   Root YR: E57B7E6F150C419102E8D5C055729FF967B9D1A829BF00CEC89CA604EBF4A86F
//   Root YE: E14FFCAD5B0025731006CAA43A121A22D8E9700F4FB9CF852F02A708AA5D5666
const tmdbGenerationYRootsPEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

func appendTMDBGenerationYRoots(pool *x509.CertPool) bool {
	if pool == nil {
		return false
	}
	return pool.AppendCertsFromPEM([]byte(tmdbGenerationYRootsPEM))
}
