...
func (t *TMDB) get(path string, q url.Values, out any) error {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		if readErr != nil {
			...
			if attempt < 3 {
				time.Sleep(t.retryDelay(attempt, r.Header))
				continue
			}
			return lastErr
		}
		...
		if r.StatusCode/100 != 2 {
			...
			if strings.TrimSpace(apiErr.StatusMessage) != "" {
				lastErr = fmt.Errorf("TMDB %s: %s", r.Status, apiErr.StatusMessage)
			} else {
				lastErr = fmt.Errorf("TMDB %s", r.Status)
			}
			if attempt < 3 && (r.StatusCode == http.StatusTooManyRequests || r.StatusCode >= 500) {
				time.Sleep(t.retryDelay(attempt, r.Header))
				continue
			}
			return lastErr
...
func (t *TMDB) Show(title string) (details, error) {
	...
	if alias := tmdbShowAliases[strings.ToLower(strings.TrimSpace(title))]; alias != "" && !strings.EqualFold(alias, title) {
