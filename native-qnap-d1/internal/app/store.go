...
func (s *Store) ReplaceScan(movies []Movie,shows []Show,episodes []Episode) error {
	...
	for i:=range movies { if old,ok:=oldMovies[movies[i].SourceURL];ok { movies[i].ID=old.ID; movies[i].TMDBID=old.TMDBID; movies[i].OriginalTitle=old.OriginalTitle; movies[i].Overview=old.Overview; movies[i].PosterURL=old.PosterURL; movies[i].BackdropURL=old.BackdropURL; movies[i].Rating=old.Rating; movies[i].Runtime=old.Runtime; movies[i].Genres=old.Genres; movies[i].MetadataStatus=old.MetadataStatus; movies[i].AddedAt=old.AddedAt } else { movies[i].ID=s.nextIDLocked(); movies[i].AddedAt=now() }; movies[i].UpdatedAt=now(); if movies[i].MetadataStatus=="" { movies[i].MetadataStatus="pending" } }
	...
	for i:=range episodes { episodes[i].ShowID=showIDMap[episodes[i].ShowID]; if old,ok:=oldEps[episodes[i].SourceURL];ok { episodes[i].ID=old.ID; episodes[i].Overview=old.Overview; episodes[i].StillURL=old.StillURL; episodes[i].Runtime=old.Runtime; episodes[i].AirDate=old.AirDate; episodes[i].MetadataStatus=old.MetadataStatus; episodes[i].AddedAt=old.AddedAt } else { episodes[i].ID=s.nextIDLocked(); episodes[i].AddedAt=now(); episodes[i].MetadataStatus="pending" }; episodes[i].UpdatedAt=now() }
	s.state.Movies=movies; s.state.Shows=shows; s.state.Episodes=episodes; return s.saveCatalogLocked()
}
func mergeShow(n,o Show) Show { n.TMDBID=o.TMDBID;n.OriginalTitle=o.OriginalTitle;n.Overview=o.Overview;n.PosterURL=o.PosterURL;n.BackdropURL=o.BackdropURL;n.Rating=o.Rating;n.Genres=o.Genres;n.MetadataStatus=o.MetadataStatus;n.AddedAt=o.AddedAt;return n }
...
func Catalog(st State) map[string]any { movies:=append([]Movie(nil),st.Movies...);shows:=make([]map[string]any,0,len(st.Shows));sort.Slice(movies,func(i,j int)bool{return movies[i].AddedAt>movies[j].AddedAt});for _,sh:=range st.Shows{seasons:=map[int]bool{};count:=0;for _,e:=range st.Episodes{if e.ShowID==sh.ID{count++;seasons[e.Season]=true}};m:=toMap(sh);m["episode_count"]=count;m["season_count"]=len(seasons);shows=append(shows,m)};return map[string]any{"movies":movies,"shows":shows} }
